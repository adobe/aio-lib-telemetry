# Advanced Grafana Setup with Custom Configuration
...
This guide demonstrates how to configure a fully customizable Grafana observability stack using individual components (Tempo, Prometheus, Loki) with a custom OpenTelemetry Collector configuration. This approach provides complete control over each component but requires more setup than the [Quick Start guide](../grafana.md).

> [!TIP]
> **When to use this guide:**
>
> - You need custom OpenTelemetry Collector configurations
> - You want fine-grained control over component settings
> - You're preparing for a production deployment with specific requirements
> - You need to customize data retention, processing, or export rules
>
> **For most use cases**, the [Docker LGTM Quick Start](../grafana.md) is simpler and sufficient.

> [!IMPORTANT]
> This guide showcases how to leverage tunneling to forward telemetry data from a remote/deployed App Builder action to a local observability stack. This is **not recommended for production environments**, and is only intended for local development while testing your App Builder actions.

- [Advanced Grafana Setup with Custom Configuration](#advanced-grafana-setup-with-custom-configuration)
...
- A tunneling tool for App Builder (see [Tunneling Guide](../support/tunnel-forwarding.md) for setup instructions) - only needed for deployed actions
